...
# Author: Daphne Chen
#***************************************************************************
# Best case sst range parameter: 9.8-10.2 C
#***************************************************************************
...
for(i in 1:100) {
	best_case_mack_df[i, 1] <- sst[[116, 1]] #  Starting location = first entry of each row
  	...
  	while(current_year < 2070 & current_block > 0) {
  		...
  		if(9.8 <= current_block_temp & current_block_temp <= 10.2) {
   		...
   		north <- current_block-20
    		...
    		if(new_block == prev_block) {
    			if(!(9.8 <= current_block_temp & current_block_temp <= 10.2) & north > 0 & !is.na(sst[[north, 2]])) {
    				new_block <- north
    			}
    		}
