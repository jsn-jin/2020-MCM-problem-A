...
# Output:
# 	  "bc_herring.csv"				ending location of herring, best case
# Author: Daphne Chen
#***************************************************************************
# Best case SST range parameter: 10.5-11.5 C
#***************************************************************************
...
for(i in 1:100) {
  ...
  while(current_year < 2070 & current_block > 0) {
    ...
    if(10.5 <= current_block_temp & current_block_temp <= 11.5) {
    ...
    if(current_block %in% left_edge) {
      ...
      adjacent <- c(north, west, south, east, northwest, 
      				southwest, southeast, northeast)
    }
    
    # average temperature of range 
    # in the case that none of the block are within range
    avg_temp <- 11
    ...
    # check whether we have data for each direction 
    # AND if the block is above or below threshold temperature
