...
# Get Summer data (the average of 6/7/8 months)
summer_avg_sst = matrix(nrow = 150, ncol = 300) # to store annually average results

for (b in 1:300) { # for each block
  
  # Prepare a vector
  b.summer = numeric(150)
  ...
  for (i in 0:149) {
    summer_sum = 0
    for (j in 6:8) {
      summer_sum = summer_sum + b.monthly[12 * i + j]
    }
    b.summer[i+1] = summer_sum / 3
  }
  
  summer_avg_sst[,b] = b.summer
}

write.csv(summer_avg_sst, file = "summer_avg_sst_150.csv")
